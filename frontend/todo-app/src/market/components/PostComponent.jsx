...
class PostComponent extends Component {
  ...
  render() {
    ...
    if (this.state.postInfo != null) {
      if (!this.state.editMode) {
        //only if we are the owner of the item
        //determines what buttons to show based on the status of the item
        var ItemButtons;
        if (this.state.postInfo.status === "AVAILABLE" && (sessionStorage.getItem('authenticatedUser') === this.state.postInfo.ownerId)) {
          ItemButtons =
            <div className="containerbuttons ">
              ...
              <input type="image" className="editBtn" alt="editImg" src={require("../images/edit.svg")} onClick={this.setEdit} />
            </div>
        }
        ...
        //displays the structure of the page when you are the owner
        retVal = (
          <div className="product">
            ...
            <div className="postDescription">
              <img className="postImages" alt="" src={`https://storage.googleapis.com/sept-image-store/${this.state.postInfo.id}`}></img>
          ...
          //displays structur eof page if youre not the owner
          retVal = (
            <div className="product">
              <PostItem existingId = {this.props.match.params.postID} history={this.props.history}></PostItem>
              <div className="cancel"><button className="btn btn-dark cancel" onClick={this.clearEdit}>Cancel</button> </div>
            </div>
          )
      }
    } else {
      //if there is no posting with that name
      retVal = null;
    }
    return retVal;
  }

  //determine whether we are editing the post
  setEdit() {
  ...
  //series of methods that tell the updateStatus methods how to change the status
  updateDelete() {
  ...
  //function that changes the status of the post
  updateStatus(status) {
    ...
    //function that permanently deletes the posting backend
  updatePermDelete() {
  ...
  //gets the data of the passed in posting
  retrieveItemInfo() {
